...
//admin dashboard without dummy data and with API integration
//-///////////////////////////////////////////////////////////////////////////
...
//     // We can add the token to the headers for every request
//     options.headers = {
//         ...options.headers,
...
// //-///////////////////////////////////////////////////////////////////////////
// // REUSABLE UI COMPONENTS (No changes here, so they are collapsed for brevity)
// //-///////////////////////////////////////////////////////////////////////////
...
//                     <div style={{ width: '100%', height: 300 }}>
//                         <ResponsiveContainer>
//                             <LineChart data={analytics.revenueData}><CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} /><XAxis dataKey="_id" stroke="rgb(107 114 128)" /><YAxis stroke="rgb(107 114 128)" /><Tooltip contentStyle={{ backgroundColor: 'rgba(31, 41, 55, 0.8)', border: 'none', borderRadius: '0.5rem', color: '#fff' }} /><Legend /><Line type="monotone" dataKey="revenue" stroke="#3b82f6" strokeWidth={2} activeDot={{ r: 8 }} /></LineChart>
//                         </ResponsiveContainer>
//                     </div>
//                 </Card>
//                 <Card>
...
//                     <div style={{ width: '100%', height: 300 }}>
//                         <ResponsiveContainer>
//                             <BarChart data={analytics.servicesData}><CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} /><XAxis dataKey="_id" stroke="rgb(107 114 128)" /><YAxis stroke="rgb(107 114 128)" /><Tooltip contentStyle={{ backgroundColor: 'rgba(31, 41, 55, 0.8)', border: 'none', borderRadius: '0.5rem', color: '#fff' }} /><Legend /><Bar dataKey="bookings" fill="#10b981" /></BarChart>
//                         </ResponsiveContainer>
//                     </div>
//                 </Card>
//             </div>
...
//                         <thead className="text-sm text-gray-500 dark:text-gray-400 uppercase bg-gray-50 dark:bg-gray-700">
//                             <tr>
//                                 <th className="p-3">Name</th><th className="p-3">Email</th><th className="p-3">Joined On</th><th className="p-3 text-center">Actions</th>
//                             </tr>
//                         </thead>
//                         <tbody>
//                             {filteredUsers.map(user => (
//                                 <tr key={user._id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/50">
//                                     <td className="p-3 font-medium text-gray-900 dark:text-white">{user.fullName}</td><td className="p-3 text-gray-600 dark:text-gray-300">{user.email}</td><td className="p-3 text-gray-600 dark:text-gray-300">{new Date(user.createdAt).toLocaleDateString()}</td>
//                                     <td className="p-3 text-center">
...
//         if (file) {
//             const newImageUrl = URL.createObjectURL(file);
//             setProfile(p => ({ ...p, profilePicture: newImageUrl, newProfilePicture: file }));
//         }
//     };
//     const handleUploadClick = () => { fileInputRef.current.click(); };
...
//             if(key !== 'newProfilePicture' && key !== 'profilePicture') {
...
//                 body: formData,
//                 headers: {
//                     'Content-Type': undefined // Let browser set content-type for multipart/form-data
//                 }
//             });
//             setCurrentUser(data);
//             setIsEditing(false);
...
//                     <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-4">
//                         <div className="lg:col-span-1 flex flex-col items-center">
//                             <img key={profile.profilePicture} src={profile.profilePicture ? `${API_BASE_URL.replace('/api/admin', '')}/${profile.profilePicture}` : `https://placehold.co/128x128/e2e8f0/4a5568?text=MotoFix`} alt="Profile" className="w-32 h-32 rounded-full object-cover mb-4 ring-4 ring-blue-500/50" onError={handleImageError} />
//                             {isEditing && (
//                                 <div className="w-full space-y-2">
//                                     <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/png, image/jpeg, image/gif" />
//                                     <Button variant="secondary" className="w-full" onClick={handleUploadClick}><Camera size={16} /> Change Picture</Button>
...
//                         </button>
//                         <div className="flex items-center gap-3">
//                             <img key={currentUser.profilePicture} src={currentUser.profilePicture ? `${API_BASE_URL.replace('/api/admin', '')}/${currentUser.profilePicture}` : `https://placehold.co/40x40/e2e8f0/4a5568?text=MF`} alt="Admin" className="w-10 h-10 rounded-full object-cover" onError={handleImageError} />
//                             <div>
//                                 <p className="font-semibold text-sm">{currentUser.ownerName}</p>
//                                 <p className="text-xs text-gray-500 dark:text-gray-400">Workshop Owner</p>
//                             </div>
